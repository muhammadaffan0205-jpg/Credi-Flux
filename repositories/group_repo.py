# crediflux/repositories/group_repo.py
...
class GroupRepo:
    @staticmethod
    def create(name: str, creator_id: int) -> Optional[int]:
        conn, cur = get_db()
        try:
            cur.execute(
                "INSERT INTO groupss (group_name, created_by) VALUES (%s, %s)",
                (name, creator_id)
            )
            ...
            cur.execute(
                """INSERT INTO group_people (group_id, user_id, display_name)
                   VALUES (%s, %s, %s)
    ...
    def delete(group_id: int) -> bool:
        ...
        try:
            cur.execute(
                """DELETE mes FROM manual_expense_shares mes
                   JOIN manual_expenses me ON mes.expense_id = me.expense_id
                   WHERE me.group_id = %s""",
                (group_id,)
            )
    ...
    def remove_person(group_id: int, person_id: int) -> bool:
        ...
        try:
            cur.execute(
                "DELETE FROM manual_expense_shares WHERE person_id = %s", (person_id,)
            )
            cur.execute(
                "DELETE FROM group_people WHERE group_id=%s AND person_id=%s",
                (group_id, person_id)
            )
            conn.commit()
    ...
    def get_people(group_id: int) -> List[Person]:
        ...
        try:
            cur.execute(
                "SELECT person_id, group_id, user_id, display_name FROM group_people WHERE group_id=%s ORDER BY person_id",
                (group_id,)
            )
    ...
    def get_member_count(group_id: int) -> int:
        ...
        try:
            cur.execute(
                "SELECT COUNT(*) AS c FROM group_people WHERE group_id=%s", (group_id,)
            )
