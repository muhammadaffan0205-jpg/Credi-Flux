# backend/repositories/payment_confirmation_repo.py
...
class PaymentConfirmationRepo:
    @staticmethod
    def create(debtor_id: int, creditor_id: int, amount: float, settlement_id=None, direct_debt_id=None) -> Optional[int]:
        conn, cur = get_db()
        try:
            cur.execute(
    ...
    def confirm(confirmation_id: int) -> bool:
        ...
        try:
            cur.execute(
                "UPDATE payment_confirmations SET status='confirmed', confirmed_at=%s WHERE id=%s",
                (datetime.now(), confirmation_id)
            )
            conn.commit()
